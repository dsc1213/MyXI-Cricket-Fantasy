...
import { canManageUser, getRulesForTournament, calculatePoints } from '../helpers.js'
import {
...
import { registerMockRoutes } from './mockRoutes.js'
...
const createRouter = ({
  ...
  jwtExpiresIn,
  mockApiEnabled = true,
  mockAutoSeedTeams = false,
  persistMockState = () => {},
}) => {
  const router = Router()
  const canonicalMockPrefixes = [
    '/page-load-data',
    '/bootstrap',
    '/tournaments',
    '/contests',
  ...
  const shouldRewriteToMock = (path = '') => {
    if (/^\/users\/[^/]+\/picks$/.test(path)) return true
    return canonicalMockPrefixes.some(
  ...
  router.use((req, res, next) => {
    ...
    res.json = (payload) => {
      res.set('X-Mock-Data', String(Boolean(mockApiEnabled)))
      if (
        payload &&
        ...
        return originalJson({ ...payload, mockData: Boolean(mockApiEnabled) })
      }
  ...
  router.use((req, res, next) => {
    if (!mockApiEnabled) return next()
    if (req.path.startsWith('/mock/')) return next()
    if (req.path.startsWith('/auth/')) return next()
    if (req.path === '/health') return next()
    if (!shouldRewriteToMock(req.path)) return next()
  ...
  const resolveMockSelection = ({
    ...
    if (!mockAutoSeedTeams && !canBackfillLegacyCompleted) return null
  ...
  router.use((req, res, next) => {
    if (!mockApiEnabled && req.path.startsWith('/mock/')) {
      return res.status(404).json({
        message: 'Mock API is disabled. Set MOCK_API=true to use /mock routes.',
      })
    }
    return next()
  })
  ...
  // In mock mode, keep /mock routes open for rapid UI/dev testing.
  // Non-mock routes still require authentication.
  router.use((req, res, next) => {
  ...
  const getContestUserPool = ({ contest, matchId = 'm1', viewerUserId = '' }) => {
