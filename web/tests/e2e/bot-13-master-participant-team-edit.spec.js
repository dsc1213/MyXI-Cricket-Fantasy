...
test.describe('13) Master participant team edit and join gate', () => {
  ...
  test('master can edit participant team from participants even after match start', async ({
    page,
    request,
  }) => {
    const suffix = `${Date.now().toString().slice(-6)}`
    const bot = {
      name: 'abc',
      gameName: `mocke2ebot-live-${suffix}`,
      email: `mocke2ebot-live-${suffix}@myxi.local`,
      password: 'demo123',
    }
    let contestId = ''
    let targetMatchId = ''
    ...
    try {
      ...
      const created = await createContest({
        request,
        tournamentId: 't20wc-2026',
        name: `E2E Master Edit Live ${suffix}`,
        teams: 2,
        createdBy: 'master',
        matchIds: [targetMatchId],
      })
      ...
      await apiCall(
        request,
        'PATCH',
        `/admin/matches/${targetMatchId}/status`,
      ...
      await expect(participantRow).toBeVisible()
      await expect(
        participantRow.getByRole('link', { name: `Edit ${bot.gameName} team` }),
      ).toBeEnabled()

      await participantRow.getByRole('link', { name: `Edit ${bot.gameName} team` }).click()
      await expect(page).toHaveURL(
        new RegExp(`/fantasy/select\\?contest=${contestId}.*userId=${bot.gameName}`),
      )
      await expect(page.getByText(`Master edit: ${bot.gameName}`)).toBeVisible()
